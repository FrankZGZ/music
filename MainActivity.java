...
//            songNameList=new ArrayList<>();
            ...
            songsize = listsong.size();
            for(int i=0;i<listsong.size();i++){
                songNameList.add(listsong.get(i).songName);
                songpathList.add(listsong.get(i).songPath);
            }
...
//                            play(path);
                            ...
                            name = listsong.get(songid).songName;
                            intent.putExtra("songpathlist",songpathList);//歌曲路径数组传到MusicActivity
